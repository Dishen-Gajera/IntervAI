...
export const generateQuestion = async (req, res) => {
    ...
    try {
        ...
        if (!role || !mode || !experience) {
            return res.status(402).json({ message: "Please Provide role, experience and mode" });
        }
        ...
        if (user.credits < 50) {
            return res.status(401).json({ message: "User have not enough credits" });
        }
        ...
        const creditLeft = await User.findByIdAndUpdate(req.userId, { $inc: { credits: -50 } }, { new: true }).select("credits");
        ...
        return res.status(200).json({
            ...
            creditsLeft: creditLeft.credits,
            userName: user.name,
        ...
        console.log(error)
        return res.status(500).json({ message: "failed generate question error" });
    }
...
export const submitAnswer = async (req, res) => {
    try {
        ...
        if (timeTaken > question.timeLimit) {
            question.score = 0;
            question.feedback = "timelimit exceeded. Answer not evaluated";
            question.answer = "";
        ...
        question.feedback = parsedAnswer.feedback;
        question.score = parsedAnswer.finalScore;
        question.confidence = parsedAnswer.confidence;
        question.communication = parsedAnswer.communication,
            question.correctness = parsedAnswer.correctness;
        question.answer = answer;

        await interview.save();
        res.status(200).json({ feedback: parsedAnswer.feedback })


    } catch (error) {
        res.status(500).json({ message: "failed submit answer error" });
...
export const finishInterview = async (req, res) => {
    try {
        ...
        console.log(error);
        res.status(500).json({ message: "failed finish interview error" });
...
export const getMyInterviews = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: `Get interview history error ${error}` });
    }
}
...
export const getInterviewReport = async (req, res) => {
    try {
        ...
        console.log(error)
        return res.status(500).json({ message: `Get interview Report error ${error}` });
    }
}