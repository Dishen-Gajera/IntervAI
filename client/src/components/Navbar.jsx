        ...
        <div className='bg-[#f3f3f3] flex justify-center px-4 pt-6'>
            <motion.div
                ...
                <div className='flex item-center gap-3 cursor-pointer'>
                    ...
                    <h1 className='hidden font-semibold md:block text-lg'>IntervAI</h1>
                ...
                <div
                    ...
                    <div className='relative'>
                        <button className='flex items-center gap-2 bg-gray-100 px-4 py-2 rounded-full text-md hover:bg-gray-200 transition' onClick={() => {
                            ...
                            setShowCreditPopup(privious => !privious); setShowUserPopup(false)
                        }}>
                            <BsCoin size={20} />
                            {userData?.credits || 0}
                        ...
                        {showCreditPopup && <div className='absolute -right-12.5 mt-3 w-64 bg-white shadow-xl border-gray-200 rounded-xl p-5 z-50'>
                    ...
                    <div className='relative'>
                        <button className='w-9 h-9 bg-black text-white rounded-full flex items-center justify-center font-semibold' onClick={() => {
                            ...
                            setShowUserPopup(prev => !prev); setShowCreditPopup(false)
                        }}>
                            {userData ? userData.name.slice(0, 1).toUpperCase() : <FaUserAstronaut />}
                        </button>

                        {showUserPopup && <div className='absolute right-0 mt-3 w-48 bg-white shadow-xl border-gray-200 rounded-xl p-4 z-50'>
                            ...
                            <button className='w-full text-left text-sm py-2 flex items-center gap-2 text-red-500'><HiOutlineLogout size={20} onClick={handleLogout} />Logout</button>
                        </div>}
                    </div>
                </div>
