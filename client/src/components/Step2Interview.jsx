...
import { motion, resolveElements, time } from 'motion/react';
...
function Step2Interview({ interviewData, onFinish }) {
  ...
  useEffect(() => {
    const loadVoice = () => {
      ...
      const maleVoice = voices.find(v =>
        v.name.toLocaleLowerCase().includes("david") || v.name.toLocaleLowerCase().includes('mark') || v.name.toLocaleLowerCase().includes("male")
      )
  ...
  useEffect(() => {
    ...
    const runIntro = async () => {
      ...
      } else if (currnetQuestion) {
        await new Promise(r => setTimeout(r, 800));
        if (currentIndex === questions.length - 1) {
          await speakText("Alright, this one might be a bit more challenging.");
        }
        await speakText(currnetQuestion.question);
  ...
  useEffect(() => {
    if (!isIntroPhase && currnetQuestion) {
  ...
  const submitAnswer = async () => {
    ...
    try {
      const result = await axios.post(serverUrl + "/api/interview/submit-answer", {
        ...
        timeTaken:
          currnetQuestion.timeLimit - timeleft
      ...
      <div className='w-full max-w-350 min-h-[80vh] bg-white rounded-3xl shadow-2xl border border-gray-200 flex flex-col lg:flex-row overflow-hidden'>
        ...
        <div className='w-full lg:w-[35%] bg-white flex flex-col items-center p-6 space-y-6 border-r border-gray-200'>
          ...
          <div className='w-full max-w-md bg-white border border-gray-200 rounded-2xl shadow-md p-6 space-y-5'>
            ...
            <div className='flex justify-center'>
              <Timer timeLeft={timeleft} totalTime={currnetQuestion?.timeLimit || 60} />
            </div>
            <div className='h-px bg-gray-200'></div>
        ...
        <div className='flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative'>
          ...
          <textarea placeholder='Type your answer here...'
            className='flex-1 bg-gray-200 p-4 sm:p-6 rounded-2xl resize-none outline-none border border-gray-200 focus:ring-2 focus:ring-emerald-500 transition text-gray-800'
